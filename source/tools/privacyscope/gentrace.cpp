...
/*
2014 07 22 KnobOut参数改为三类输出文件的前缀
如前缀为1-1，则输出如下
tracefile命名为1-1-trace.bpt
辅助文件命名为1-1-assist.assist
异常信息文件命名为1-1-exception.exception
*/
...
KNOB<int> KnobEndAddr(KNOB_MODE_WRITEONCE, "pintool",
                       "end_addr", "",
                       "Address of trigger point. No logging will occur until execution reaches this address.");
...
//9 20 liu
KNOB<time_t> KnobTimeLimit(KNOB_MODE_WRITEONCE, "pintool",
                            "time-limit", "0",
                            "time out seconds.");
...
//2014 10 17 记录所有的基本块地址，写入文件
uint32_t g_BBLAddrArray[4096];
uint32_t g_BBLidx;
//FILE *fpaddrs;
...
// could be tainted.
bool g_active;//当污点引入时，激活
//bool g_beginToLog;//当条件满足时，开始记录 
...
time_t start_time ;
time_t g_time;
uint64_t g_counter = 0;//该值为所有执行的指令数量
...
static uint32_t GetBitsOfReg(REG r)
{
    ...
    if (REG_ST_BASE <= r && r <= REG_ST_LAST)
		return 80;
...
VOID Activate(CONTEXT *ctx)
{
    ...
    g_active = true;
//	g_beginToLog = true;//added by 0319
...
{
    cerr << "Deactivating logging" << endl;
    g_active = false;
	g_beginToLog = false;//added by 0319
    PIN_RemoveInstrumentation();
    PIN_ExecuteAt(ctx);
}*/
...
{
	ofstream OutFile;
  OutFile.open("d:\\log.txt",std::ios::out|std::ios::ate);
  OutFile.setf(ios::showbase);
  OutFile << "enter tactivate "  << endl;
  OutFile.close();
    cerr << "Activating taint analysis " << endl;
    //g_active = true; /* Any instruction could be logged because taint is introduced. */
...
{
	int error;
	//error = fwrite(g_BBLAddrArray,sizeof(uint32_t), g_BBLidx ,fpaddrs);
	cerr <<error<<"  "<<g_BBLidx<<endl;
	g_BBLidx = 0;
}
...
ADDRINT CheckBuffer(UINT32 count,UINT32 addr)
{
	//g_BBLAddrArray[g_BBLidx++] = addr;
	return ((g_bufidx + count) >= BUFFER_SIZE - FUDGE)/*||(g_BBLidx==sizeof(g_BBLAddrArray)/4)*/;
}

set <ADDRINT> g_bbls;
// maximum size of the buffer.
ADDRINT WriteBlock(THREADID threadid,UINT32 addr)
{
	//GetLock(&lock, threadid+1);
	/*g_BBLAddrArray[g_BBLidx++] = addr;
	if(g_BBLidx == sizeof(g_BBLAddrArray)/4)
	{
		//cerr<<g_BBLidx-1<<" flushaddrs"<<"\n";
		FlushAddrs();
	}*/
...
// context.
// needlock 除了表示加锁外，还表示是在 BBL_InsertThenCall后调用的
VOID FlushBuffer(bool addKeyframe, const CONTEXT *ctx, THREADID threadid, bool needlock,UINT32 count)
{
	...
	if(g_BBLidx == sizeof(g_BBLAddrArray)/4)
	{
		cerr<<g_BBLidx<<" flushaddrs"<<"\n";
		FlushAddrs();
	}*/
	if((g_bufidx + count) >= BUFFER_SIZE - FUDGE || count==0)
	{
		FlushInstructions();
...
void* GetEnvWWrap(CONTEXT *ctx, AFUNPTR fp, THREADID tid) 
{
    ...
    /* We must lock after the PIN_CallApplicationFunction call, since
       the called code is instrumented, and also tries to obtain the lock. 
...
VOID AppendBuffer(ADDRINT addr,THREADID tid,
	CONTEXT *ctx,bool isBranch,
	UINT32 insn_length, UINT32 rawbytes0,
	UINT32 rawbytes1,
	UINT32 rawbytes2,
	UINT32 rawbytes3,
	...
	...)
{
	...
	{
		cout << "Logged required number:"<<g_Execlimit<<"  "<<g_counter<<" instructions, quitting.\n";
		ofstream OutFile;
							  OutFile.open("c:\\test\\log.txt",ios::out | ios::app);
							  OutFile.setf(ios::showbase);
							  OutFile << "ins out"<< endl;
							  OutFile.close();
		Cleanup();
        //ReleaseLock(&lock);
        // Never release lock
	...
	bool log_all = ((LogAllAfterTaint.Value() && !firstTaint)|| LogAllBeforeTaint.Value());
	...
	{//污点传播、记录
		...
		* instructions */
		...
		//added by 0319 只有涉及污点的指令才记录
		//bool propagated_taint = tracker->propagatedTaint(isBranch);//0510应放到下一指令之后
		//	 0504 支持记录跳转指令，屏蔽下条指令
		//if (!isBranch)
		{
			tracker->taintPropagation(ti->delta);
		}
		bool propagated_taint = tracker->propagatedTaint(isBranch);
		// Taint checking
		//防止程序中途退出，eip被污染
		...
		//决定是否记录
		//if (g_beginToLog && (has_taint && propagated_taint))
		//0530,只要hastaint，就记录,写污点也记录 增加propagated_taint 则只记录污点传播指令
		if (g_active && (has_taint /*&& propagated_taint*/))
		{
			...
			//0430
			tracker->printRegs(ti->delta);
			//cerr << "inst "<< rawbytes0<<" " <<rawbytes1<<" " <<rawbytes2<<" " <<rawbytes3;
			cerr <<"addr " << addr << " disa "<<rawbytes0<<" " <<rawbytes1<<" ";
			...
			for (unsigned int i = 0; i < values_count; i++)
			{
				...
				if(values[i].type.type == REGISTER)
				{
					...
					{
					case P_CONTEXT:
						g_buffer[g_bufidx].valspecs[i].value.dword[0] =	PIN_GetContextReg(ctx, r);
						break;
					case P_REF:
						pr = (LEVEL_VM::PIN_REGISTER*) values[i].value.dword[0];
						memcpy(&(g_buffer[g_bufidx].valspecs[i].value),pr,sizeof(LEVEL_VM::PIN_REGISTER));
						break;
					case P_FPX87:
						if(!got_FP_state)
						{
						...
						if (s_i > 7)
						{
							cerr << "Unknown FP register " << r << " at addr "<< addr << endl;
							assert(false);
						}
						fpValue = (void *)&(fpState.fxsave_legacy._sts[s_i]);
						memcpy(&(g_buffer[g_bufidx].valspecs[i].value.flt[0]),fpValue,10); 
						// FP are 80 bits = 10 bytes
						break;
					default:
						assert(false);
						break;                                                   
				...
				{
					cerr << "Unknown operand type at addr " << addr << endl;                                          
					assert(false);                                                 
				}
	...
	if (abort && !SEHMode.Value())
	{
		...
		* this so it works for any last instruction. */
		...
		FlushBuffer(true, ctx, tid, false,0);
		Cleanup();
		exit(0);
	}
	if (g_exit_next)
...
VOID InstrBlock(BBL bbl)
{
	...
	bool hasreps;
	// LOG("INS: BBL start.\n");
	// cerr << "g_active test: " << g_active << endl;
	//if (g_active)
	//{
		if (icount > BUFFER_SIZE)
		{
			LOG("WARNING: Basic block too many instructions: " + decstr(icount) + "\n");
			assert(false);
		}
		...
		BBL_InsertIfCall(bbl, IPOINT_BEFORE,(AFUNPTR) CheckBuffer,IARG_UINT32, icount,IARG_UINT32, addr,IARG_END);

		BBL_InsertThenCall(bbl, IPOINT_BEFORE,(AFUNPTR) FlushBuffer,IARG_BOOL, true,IARG_CONTEXT,IARG_THREAD_ID,IARG_BOOL, true,IARG_UINT32, icount,IARG_END);
	//}
	...
	for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
	{
		if (!g_active && g_usetrigger)
		{
			...
			if (INS_Address(ins) == g_trig_addr) 
			{
				...
				char buff[1024];
				sprintf(buff,"addr triggerd %x",INS_Address(ins));
				LOG(buff);
				INS_InsertIfCall(ins,   IPOINT_BEFORE, (AFUNPTR)CheckTrigger,IARG_END);
				INS_InsertThenCall(ins, IPOINT_BEFORE,(AFUNPTR) Activate,	IARG_CONTEXT,IARG_END);
			}
			// Skip the rest of the analysis and immediately go on to the next instruction.
			continue;
		}
		//20140510 引入结束条件
		/*if (INS_Address(ins) == g_end_addr) 
		{
			// Found the trigger address.
			char buff[1024];
			sprintf(buff,"end addr triggerd %x",INS_Address(ins));
			LOG(buff);
		//遇到某指令，退出
			INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)PostInstruction, IARG_END);
	//		INS_InsertIfCall(ins,   IPOINT_BEFORE, (AFUNPTR)CheckEnd,IARG_END);
	//		INS_InsertThenCall(ins, IPOINT_BEFORE,(AFUNPTR) DeActivate,	IARG_CONTEXT,IARG_END);
		}*/
		// Skip instrumentation unless g_active is enabled
		//if (!g_active)
		//by xg  0319 g_active 用于指令匹配结果，开始记录  g_taint_introduced 开始污点传播
		if(!g_taint_introduced)
		{
			//LOG("continued \n");
			continue;
		}
		//cerr <<"addr " << INS_Address(ins) << "disa "<<INS_Disassemble(ins)<<" ";
		...
		else if ( INS_Category(ins) == XED_CATEGORY_PREFETCH )
		{
			...
			cerr << "Not logging prefetch instruction @" << INS_Address(ins) << ": "
		...
		else if (INS_Category(ins) == XED_CATEGORY_MMX)
		{
			...
			cerr << "Not logging mmx instruction @" << INS_Address(ins) << ": "
				<< INS_Disassemble(ins) << endl;
			continue;
		}
		...
		IARGLIST_AddArguments(arglist,IARG_UINT32, rawbytes_i[0],
			...
			INS_Mnemonic(ins) == string("PXOR"))
		{
			...
			if (found)
			{
				...
				{
					if (INS_OperandReadAndWritten ( ins, i) &&
						INS_OperandIsReg( ins, i ) && r == INS_OperandReg( ins, i ) 
						&&(unsigned)opnum != i)
					{
						is_xor = true;
						break;
		...
		for(uint32_t i = 0; i < INS_OperandCount(ins); i++)
		{
			...
			if (INS_OperandIsReg(ins, i))
			{
				...
				if(hasreps && r==REG_ECX)//0530 REPS MOVS 指令，ecx不被标记为写
				{
					opndvals[valcount].taint &= ~WR;
				}
				if(r == REG_INVALID())
			...
			else if (INS_OperandIsMemory(ins, i)|| INS_OperandIsAddressGenerator(ins,i)) 
			{
		...
		* operands to make this explicit. */
		...
		IARGLIST_AddArguments(arglist,IARG_IARGLIST, arglist_helper,IARG_END);
		...
		/*if(INS_Address(ins)== 0x3015ddb5)
		{
			cerr << "0x3015ddb5" << endl;
			cerr << g_end_addr << endl;
		}*/
		if ((LogOneAfter.Value() && !(INS_Address(ins) >= start_addr && INS_Address(ins) <= end_addr))
			||INS_Address(ins) == g_end_addr)
		{
...
VOID InstrTrace(TRACE trace, VOID *v)
{
	...
	else
	{
		for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
		{
			...
			if(bwriteaddr)
			{
				BBL_InsertCall(bbl, IPOINT_BEFORE,(AFUNPTR) WriteBlock,IARG_THREAD_ID,IARG_UINT32, addr-DllbaseAddress,IARG_END);
			}
		}
    }
}
    ...
    // Try to find kernel32
    {
        ...
        if (lasttok)
		{
			...
			else if (strncmp(wsDll, lasttok, BUFSIZE) == 0)
			{
				...
				{
                    PROTO proto = PROTO_Allocate(PIN_PARG(int), CALLINGSTD_STDCALL,
                                                 "WSAConnect",  PIN_PARG(uint32_t),
                                                 PIN_PARG(void*),  PIN_PARG(void*),
                                                 PIN_PARG(void*),  PIN_PARG(void*),
                                                 PIN_PARG(void*),  PIN_PARG(void*),PIN_PARG_END());
  ...
  //  if (g_usetrigger && !g_trig_resolved)
	{
        ...
        if ((KnobTrigModule.Value() == "") ||name.find(KnobTrigModule.Value()) != string::npos)
		{
            // Found the module, resolve address.
			 ...
			 if(g_end_addr==0)
			 {
				 g_end_addr  = KnobEndAddr.Value() +  IMG_LoadOffset(img);
				 char buff[1024];
				 sprintf(buff,"g_trig_addr %x  g_end_addr %x  IMG_LoadOffset %x\n",
					 g_trig_addr,g_end_addr, IMG_LoadOffset(img));
				 LOG(buff);
			 }
...
VOID SyscallEntry(THREADID tid, CONTEXT *ctx, SYSCALL_STANDARD std, VOID *v)
{
	...
	int a=PIN_GetSyscallNumber(ctx, std);
...
VOID SyscallExit(THREADID tid, CONTEXT *ctx, SYSCALL_STANDARD std, VOID *v)
{
    ...
    // sytem call
    FrameOption_t fo = tracker->taintPostSC(PIN_GetSyscallReturn( ctx, std ),
		(const uint64_t*) (si.sf.syscall_frame().argument_list().elem().data()), addr, length, si.state);
    if (fo.b)
	{
        if (!g_taint_introduced)
		{
...
address:0x10001cc8
Registers:EAX = 0x0012dc10  EBX = 0x10036b2c  ECX = 0x0000000f  EDX = 0x0012dce6  ; ESI = 0x53d165d8  EDI = 0x0012dcc8  EBP = 0x53d165f6  ESP = 0x0012dbd0 ;  EIP = 0x10001cc8  CS  = 0x1b        DS  = 0x23        FS  = 0x3b       ; 
code:rep movsd
module:HookException.dll
*/
...
int LogExcepAsmCode(UINT  dwExceptionAddress,FILE * fp)
{
	...
	char				szInstString[512];
//	char				szModuleName[MAX_MODULE_NAME32 + 1] = {0};
	//CString				strModuleName;
	unsigned int		dwIndex, dwDisp, dwLength;
...
#define		CODE_LENGTH2	60
	...
	//memcpy(szBuffer, (void *)(dwExceptionAddress - CODE_LENGTH), CODE_LENGTH2);
	PIN_SafeCopy(szBuffer,(void *)(dwExceptionAddress - CODE_LENGTH), CODE_LENGTH2);
	
	get_instruction(&inst, &szBuffer[CODE_LENGTH], MODE_32);
	get_instruction_string(&inst, FORMAT_INTEL, 0, szInstruction, sizeof(szInstruction));
	
	sprintf(buffer,"code:%s\n",szInstruction);
	fwrite(buffer,1,strlen(buffer),fp);			
	
	//ExceptionNode.GetChild("Code")->SetValue(szInstruction);
	//strModuleName = GetModuleInfo(dwExceptionAddress, szModuleName);
	//ExceptionNode.GetChild("Module")->SetValue(strModuleName);

	dwDisp = 0;
	while (dwDisp < CODE_LENGTH)
	{
		dwIndex = dwDisp;
		while (dwIndex < CODE_LENGTH2)
		{
			dwLength = get_instruction(&inst, &szBuffer[dwIndex], MODE_32);
			if (dwLength == 0)
				break;

			dwIndex += dwLength;
			if (dwIndex == CODE_LENGTH)
			{
				break;
			}
	...
	if (dwDisp >= CODE_LENGTH)
	{
		dwDisp = CODE_LENGTH;
	}

	dwIndex = dwDisp;
	dwExceptionAddress -= CODE_LENGTH;
	instbuff[0] = 0;
	while (dwIndex < CODE_LENGTH2)
	{
		dwLength = get_instruction(&inst, &szBuffer[dwIndex], MODE_32);		
		if ((dwLength == 0) || ((dwIndex + dwLength) >= CODE_LENGTH2))
			break;
		...
		if (dwIndex == CODE_LENGTH)
		{
			sprintf(szInstString, "0x%08x :  %s  <--*** ; \r\n", dwExceptionAddress + dwIndex, szInstruction);
		}
		else
		{
			sprintf(szInstString, "0x%08x :  %s  ; \r\n", dwExceptionAddress + dwIndex, szInstruction);
		}
		strcat(instbuff,szInstString);
		//buff += szInstString;
		dwIndex += dwLength;
	}

	//ExceptionNode.GetChild("Codes")->SetValue(buff);

	sprintf(buffer,"codes:%s\n",instbuff);
	fwrite(buffer,1,strlen(buffer),fp);			
	return TRUE;
}
...
VOID ExceptionHandler(THREADID threadid, CONTEXT_CHANGE_REASON reason,
	const CONTEXT *from, CONTEXT *to, INT32 info, VOID *v)
{
    ...
    FlushBuffer(false, from, threadid, false,0);
    g_twnew->add(f);
	...
	else if (reason == CONTEXT_CHANGE_REASON_EXCEPTION) 
	{
		...
		if(info == accessViolation )
		{
			stringstream ss;
			ss <<"C:\\Test\\"<< KnobOut.Value()<<"-"<<"exception.txt";
			FILE *fp = fopen(ss.str().c_str(), "wb");
			sprintf(buffer,"exceptioncode:0x%x\r\n",info);
			fwrite(buffer,1,strlen(buffer),fp);
			sprintf(buffer,"address:%x\r\n",pc);
			fwrite(buffer,1,strlen(buffer),fp);
			sprintf(buffer,"Registers:EAX =%x  EBX = %x  ECX = %x   EDX = %x  ; "
				"ESI = %x  EDI = %x  EBP = %x  ESP = %x ;EIP = %x    CS  = %x   "
        ...
        if (info == accessViolation && SEHMode.Value() && g_taint_introduced)
		{
            ...
            ADDRINT eptr = PIN_GetContextReg(to, REG_SEG_FS_BASE) + ehandler_fs_offset;
...
VOID Cleanup()
{
	g_twnew->finish();
	stringstream ss;
	ss <<"C:\\Test\\"<< KnobOut.Value()<<"-"<<"assist.txt";
	FILE *fp = fopen(ss.str().c_str(), "wb");
	fwrite(g_TaintAsistBuff, 4, g_tsbufidx + 1, fp);
	fclose(fp);

	/*fwrite(g_BBLAddrArray,sizeof(uint32_t), g_BBLidx, fpaddrs);
	g_BBLidx = 0;*/
	for(set<ADDRINT>::iterator iter=g_bbls.begin();iter!=g_bbls.end();++iter)
	{ 
		fpaddrs<<hex<<*iter<<endl;
	}
	//fclose(fpaddrs);
    fpaddrs.close();
    LOG("done.\n");
...
int main(int argc, char *argv[])
{
    ...
    if (KnobTrigAddr.Value() != 0)
	{
		...
		char buff[1024];
		sprintf(buff,"g_trig_addr %x  KnobTrigAddr %x \n",	g_trig_addr,	KnobTrigAddr.Value());
		LOG(buff);
		//liu?    g_trig_addr is not assigned.
	...
	/*RTN dispatchRtn = RTN_FindByName(img, "read");
	//插桩写文件函数
    ...
    if (KnobTaintTracking.Value()) 
	{
		...
		cerr << TaintedOffsets.NumberOfValues();
		cerr << NumOffsetsParams;
		cerr <<  "NumOffsetsParams";
		cerr << "\n";
		//cout<< NumOffsetsParams;
    ...
    PIN_AddFollowChildProcessFunction(FollowExec, 0);//pin -follow_execv
    PIN_AddSyscallEntryFunction( SyscallEntry, 0 );
    PIN_AddSyscallExitFunction(SyscallExit, 0);
    PIN_AddFiniFunction(Fini, 0);

	// ss << PIN_GetPid() << "-" << KnobOut.Value();
	//ss <<"C:\\Files\\TraceFiles\\"<< KnobOut.Value()<<"-"<<"trace.bpt";
	ss <<"C:\\Test\\"<< KnobOut.Value()<<"-"<<"trace.bpt";
	...
	ss2 <<"C:\\Test\\"<< KnobOut.Value()<<"-"<<"addrs.txt";
	cerr << ss2.str().c_str()<<"\n";
	fpaddrs.open(ss2.str().c_str());
	//fpaddrs = fopen(ss2.str().c_str(), "wb");
	g_BBLidx = 0;
    g_kfcount = 0;
    g_logcount = 0;
    g_loglimit = KnobLogLimit.Value();
