...
int get_real_instruction2(unsigned char *addr, int *flags)
{
	switch (*addr) {
...
int get_real_instruction(unsigned char *addr, int *index, int *flags) {
	switch (*addr) {
...
int get_operand(PINST inst, int oflags, PINSTRUCTION instruction,
	POPERAND op, unsigned char *addr, enum Mode mode, int iflags) {
	...
	switch (MASK_AM(oflags)) {
		...
		case AM_E:
			...
			switch (dispbytes) {
				case 0:
					break;
				case 1:
					op->displacement = *(unsigned char *)(addr + 1 + index);
					// Always sign-extend
					if (op->displacement >= 0x80)
						op->displacement |= 0xffffff00;
					break;
				case 2:
					op->displacement = *(unsigned short *)(addr + 1 + index);
					// Malformed opcode
					if (op->displacement < 0x80)
						return 0;
					break;
				case 4:
					op->displacement = *(unsigned int *)(addr + 1 + index);
		...
		case AM_I:
			...
			switch (MASK_OT(oflags)) {
				case OT_b:
					op->immbytes  = 1;
					op->immediate = *(unsigned char *)(addr + index);
				...
				case OT_v:
					...
					op->immediate = (mode == MODE_32) ? 
						...
						*(unsigned short *) (addr + index);
					break;
				case OT_w:
					op->immbytes  = 2;
					op->immediate = *(unsigned short *)(addr + index);
					break;
			}
		...
		case AM_A:
			...
			op->displacement = (mode == MODE_32) ?
				*(unsigned int *)addr : *(unsigned short *) addr;
			op->section = *(unsigned short *)(addr + op->dispbytes - 2);
		...
		case AM_O:
			...
			switch (MASK_OT(oflags)) {
				case OT_b:
					op->dispbytes    = 1;
					op->displacement = *(unsigned char *)addr;
					break;
				case OT_v:
...
int get_operand_string(INSTRUCTION *inst, OPERAND *op,
	enum Format format, unsigned int offset, char *string, int length) {
...
int get_instruction(PINSTRUCTION inst, unsigned char *addr, enum Mode mode) {
	PINST ptr;
	int index = 0;
	int flags = 0;
	const char *ext = NULL;
...
int get_operands_string(INSTRUCTION *inst, enum Format format, unsigned int offset,
...
int get_instruction_string(INSTRUCTION *inst, enum Format format, unsigned int offset,
		char *string, int length) {
...
int get_operand_immediate(POPERAND op, unsigned int *imm) {
	if (op->immbytes) {
		*imm = op->immediate;
		return 1;
	} else
...
int get_operand_displacement(POPERAND op, unsigned int *disp) {
	if (op->dispbytes) {
