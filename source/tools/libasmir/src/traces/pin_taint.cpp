...
TaintTracker::TaintTracker(ValSpecRec * env)
  : taintnum(1), values(env),taint_net(false), taint_args(false),pf(defaultPolicy)
{
#ifdef _WIN32
		...
		for (unsigned int i = 0; i < length; i++)
		{
			...
			else
			{
				...
				else
				{
					...
					g_tsbufidx++;
					*(int *)g_TaintAsistBuff = g_tsbufidx;
					//cerr << "adding new mapping from " << source << " to"  << offset+i<<":"<< *(char *)(addr +i)<< " on taint num " << t << endl;
				}
			}

			stringstream ss;
			ss <<"c:\\test\\"<< KnobOut.Value()<<"-"<<"assist.txt";
...
bool TaintTracker::taintPreSC(uint32_t callno2, const uint64_t *args, /* out */ uint32_t &state)
{
  ...
  case __NR_createfilewin:
    {
      ...
      cerr << "In createfilewin " << callno2<<" "<<tempstr<<endl;
	  ...
	  transform(tempcppstr.begin(),tempcppstr.end(),tempcppstr.begin(),::tolower);
...
FrameOption_t TaintTracker::taintPostSC(const uint32_t bytes, 
	const uint64_t *args,
	uint32_t &addr,
	uint32_t &length,const uint32_t state)
{
	//for ( int i = 0 ; i < MAX_SYSCALL_ARGS ; i ++ )
	//cout << hex << " " << args[i] ;
	//cout << endl ;
	...
	case __NR_mmap2:
		{
			...
			cerr << "Tainting " << length << "bytes from mmap" << endl;
	...
	case __NR_readfilewin:
		if (bytes == STATUS_SUCCESS)
		{
			FrameOption_t fb = FrameOption_t(false);
			...
			if (psb->Information)
			{
				...
				addr = args[5];
				assert(addr);
				if(taint_sources.size()>0)
				{
					for (pos = taint_sources.begin(); pos != taint_sources.end();pos++) 
					{
						int lower = pos->first > fds[args[0]].offset ? pos->first:fds[args[0]].offset;
						int upper = pos->first + pos->second < fds[args[0]].offset +length? pos->first + pos->second:fds[args[0]].offset +length;

							cerr << "pos->first "<<pos->first <<"pos->second "<<pos->second<<
								"fds[args[0]].offset "<<fds[args[0]].offset << "length " << length<<endl;
						if(upper >=lower)
						{
							cerr << "Tainting " << upper-lower << " bytes from read @" << lower << endl;
							assert(fds.find(args[0]) != fds.end());
	...
	case __NR_mapviewofsectionwin:
		if (bytes == STATUS_SUCCESS)
		{
			...
			assert(addr);
			cerr << "Tainting " << length << " bytes from read @ from map view" << addr << endl;
			std::set<taint_s>::iterator pos;
			if(taint_sources.size()>0)
			{
				for (pos = taint_sources.begin(); pos != taint_sources.end();pos++) 
				{
					if( pos->first + pos->second <length)
					{
						for(int i =0; i<pos->second; i++)
						{
							g_TaintAsistBuff[ g_tsbufidx + 1 ] = pos->first+i;
							g_tsbufidx++;
							*g_TaintAsistBuff = g_tsbufidx;
						}
						fb = introMemTaint(addr + pos->first, pos->second, "read", -1);
					}
				}
			}
			return fb;
	...
	for (uint32_t i = 0 ; i < count ; i++)
	{
		if ((values[i].usage & WR) == WR)
		{
			if (isReg(values[i].type))
			{
				...
				if(loc == REG_INST_PTR && memreadindex== 0xffffffff )
				{}
				else
				{
			...
			else if (isMem(values[i].type))
			{
				...
				cerr <<" memwriteindex=" << memwriteindex << " memreadindex=" <<memreadindex<< endl ;
				loc = values[i].loc;
				uint32_t size = getSize(values[i].type);
				if(memreadindex != 0xffffffff && memreadindex != memwriteindex)
				{
					loc_orig = values[memreadindex+(i - memwriteindex)].loc;
					if(size == getSize(values[memreadindex+(i - memwriteindex)].type)&& loc_orig != loc)
					{
						for(uint32_t j = 0 ; j < size ; j++)
						{
							tagi = NOTAINT;
							uint32_t status = getTaint(memory, loc_orig + j);
							tagi = combineTaint(tagi, status);
							if(status!=0)
							{
								cerr << " from memory " << loc_orig + j <<" "<<j<<" to "<<loc+j<< endl;
							}
							setTaint(memory,loc+j,status);
						}
						values[i].taint = tagi;
						continue;
					}
