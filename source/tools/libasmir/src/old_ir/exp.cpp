...
string	BinOp::optype_to_string(const binop_type_t binop_type)
{
  return strs[binop_type];
}

string	BinOp::optype_to_name(const binop_type_t binop_type)
{
  return binopnames[binop_type];
}


binop_type_t	BinOp::string_to_optype(const string s)
{
...
string	UnOp::optype_to_string(const unop_type_t op)
{
  // Do NOT change this. It is used in producing XML output.
  string ret;
  switch(op){
...
unop_type_t	UnOp::string_to_optype(const string s)
{
  if(s == "NEG") 
	  return NEG;
  if(s == "NOT") 
